...
def _plot_monitors(pyht_bunch, xsuite_bunch, pyht_slices, xsuite_slices):
    ...
    axes[1, 2].plot(turns, pyht_slice_max, ".-", color="tab:blue", label="PyPARIS_sim_class")
    axes[1, 2].plot(turns, xsuite_slice_max, ".-", color="tab:orange", label="PyEC4XS")
    axes[1, 2].set_title("max slice population")
...
def main():
    ...
    summary = {
        ...
        "sigma_z_max_abs_diff": float(np.max(np.abs(pyht_bunch.sigma_z - xsuite_bunch.sigma_z))),
        ...
        "initial_mean_dp_abs_diff": float(abs(pyht_bunch.mean_dp[0] - xsuite_bunch.mean_dp[0])),
        "initial_mean_z_abs_diff": float(abs(pyht_bunch.mean_z[0] - xsuite_bunch.mean_z[0])),
