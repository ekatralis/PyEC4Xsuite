...
def _load_module(param_file):
    spec = importlib.util.spec_from_file_location("temp.mod", param_file)
...
class SimConfig:
    def __init__(self, param_file, fname_pkl_at_update="./sim_param.pkl"):
    ...
    def update(self, param_file):
        ...
        absfol_mod = os.path.dirname(abspath_mod)
        ...
        for nn in dir(mod_params):
            ...
            try:
                ...
                if "!folder_of_this_file!" in vv:
                    setattr(self, nn, vv.replace("!folder_of_this_file!", absfol_mod))
            except Exception:
                pass

        self.loaded_files.append(abspath_mod)
